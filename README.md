...
3. Copy and paste the [onEdit.gs](onEdit.gs) script into the window. *Change the lambda_url to reflect the url generated when you created the lambda function.*
...
### Lacking IFTTT
If you use IFTTT to build the Google Sheet, the onEdit function won't run during those changes, but you could use a Time-driven trigger although the [onEdit.gs](onEdit.gs) would need to be changed.
### Debugging
