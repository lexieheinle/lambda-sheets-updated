...
function getSpreadsheet(id, sheet_num = 0, duration = 5) {
  ...
  return doc
    .useServiceAccountAuthAsync(credentials)
    .bind(this)
    .then(() => {
      ...
      return doc
        ...
        .then(info => {
          let title =
            info.title.toLowerCase().replace(/ /g, "_") +
            "-" +
            info.worksheets[parseInt(sheet_num)].title
              .toLowerCase()
              .replace(/ /g, "_");
          return {
      ...
      if (sheetInfo) {
        ...
        return sheet
          ...
          .then(cells => {
            cells.forEach(cell => {
              ...
              if (clean_header.indexOf("#") != -1) {
                ...
                headers[cleaner_header] = cell.value.trim();
              } else {
                headers[clean_header] = cell.value.trim();
              }
            });
            return headers;
          })
          .then(() => {
            return sheet.getRowsAsync().then(rows => {
              rows.forEach(row => {
                let rowInfo = {};
                Object.keys(headers).forEach(header => {
                  let weird_header = header.replace(/_/g, "");
          ...
          .then(() => {
            ...
            const json_name = sheetInfo.title + ".json";
            json_spreadsheet["headers"] = headers;
